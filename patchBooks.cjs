...
const userBooksRaw = fs.readFileSync('user_books.json', 'utf8');
const userBooks = JSON.parse(userBooksRaw);

const booksTsStr = fs.readFileSync('src/data/books.ts', 'utf8');
const booksMatch = booksTsStr.match(/export const books: Book\[\] = (\[[\s\S]+\]);/);
if (!booksMatch) {
...
books.forEach(b => {
  const altId1 = b.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
  const altId2 = b.title.toLowerCase().replace(/[^a-z0-9]+/g, '');
  
  let target = userBookMap.get(b.id) || userBookMap.get(altId1) || userBookMap.get(altId2);
  ...
  if (b.description) {
    ...
    // Write energetic description if it's missing
